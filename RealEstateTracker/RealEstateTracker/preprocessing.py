...
def neighborhood(df):
    ...
    df["neighborhood"] = new[1].apply(lambda x: x.split(", ")[-1])
    return df.drop(columns="ndirty")
...
def preprocess():
    ...
    df = pd.read_csv(os.path.dirname(preprocessing.__file__) + "/data/real_estate_data.csv",index_col="Unnamed: 0").drop(columns="Unnamed: 0.1")
    df = df.drop(columns=["elevator","heating","floor"])
    df = surface(df)
    df = neighborhood(df)
    df = price(df)
    df = rooms(df)
    df = df[df["neighborhood"]!=" Madrid Capital"]
